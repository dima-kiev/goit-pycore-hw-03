...
PATTERN_NOT_DIGITS = "[^\\d]"
PATTERN_LAST8 = "[0]?\\d{9}$"


def normalize_phone(phone_number):
    sanitized = re.sub(PATTERN_NOT_DIGITS, "", phone_number)
    print(sanitized)
    matched = re.search(PATTERN_LAST8, sanitized)
    if matched:
        return "+38" + matched.group(0)
    return ""
...
good_numbers = []
for possible_number in raw_numbers:
    after_check_number = normalize_phone(possible_number)
    if after_check_number != "":
        good_numbers.append(after_check_number)


print(good_numbers)
assert len(good_numbers) == 9
...
# та перетворює його на стандартний формат, залишаючи тільки цифри та символ '+'. Якщо номер не містить міжнародного
# коду, функція автоматично додає код '+38' (для України). Це гарантує, що всі номери будуть придатними для відправлення SMS.
...
#Якщо міжнародний код відсутній, функція додає код '+38'. Це враховує випадки, коли номер починається
# з '380' (додається лише '+') та коли номер починається без коду (додається '+38').
...
#Коректність роботи функції: функція має правильно обробляти різні формати номерів, враховуючи наявність
# або відсутність міжнародного коду.
