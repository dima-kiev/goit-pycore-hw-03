...
def get_upcoming_birthdays(birthdays):
    ...
    for person in birthdays:
        birthday = datetime.strptime(str(today.year) + "." + birthdays[person], '%Y.%m.%d')
        delta = (birthday - today).days
        if 0 < delta < 7:
            if birthday.weekday() == 5:
                birthday = birthday + timedelta(2)
                print("log: moved from saturday " + str(birthday))
            elif birthday.weekday() == 6:
                birthday = birthday + timedelta(1)
                print("log: moved from sunday " + str(birthday))
            found.append(birthday)
    return found


birthdays = {"Kolya": "07.15",
             "Vasya": "07.30",
             "Nadya": "08.03",
             "Ira": "08.13"}

greetings = get_upcoming_birthdays(birthdays)
print(greetings)
...
# Параметр функції users - це список словників, де кожен словник містить ключі name (ім'я користувача, рядок) та birthday
# (день народження, рядок у форматі 'рік.місяць.дата').
...
# (дата народження у форматі рядка 'рік.місяць.дата'). Ви повинні перетворити дати народження з рядків у об'єкти
# datetime. Конвертуйте дату народження із рядка у datetime об'єкт -
# datetime.strptime(user["birthday"], "%Y.%m.%d").date(). Оскільки потрібна лише дата (без часу),
